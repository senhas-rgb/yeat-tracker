...
export const sources = [
 ...
 {id:'reddit',subreddit:'yeat_',name:'Reddit · r/yeat_',url:'https://www.reddit.com/r/yeat_/new/.rss',fallbackUrl:'https://fetchrss.com/feed/1x7Gmi04PE7O1x7GmJ4uj0mI.rss'},
];
...
export function createFeedReader({fetcher=fetch,now=Date.now}={}){
 ...
 async function readSource(source){
  ...
  try{
    ...
    let format='xml';
    if(source.fallbackUrl && response.status===429){
     response=await fetcher(source.fallbackUrl,{signal:AbortSignal.timeout(8000),headers:{'User-Agent':'LyfeNewsTracker/1.0 (public RSS reader)','Accept':'application/rss+xml, application/xml, text/xml'}});
    }
