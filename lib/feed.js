...
import {clusterStories,inferCategory} from './story-intel.js';
...
export function normalize(xml, source){
 ...
 return items.filter(item=>/\byeat\b/i.test(plain(item.title)+' '+plain(item.description))).flatMap(item=>{
  ...
  const description=plain(item.description).replace(new RegExp(`^${cleanTitle.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\s*`,'i'),'').replace(new RegExp(`\\s*${publisher.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\s*$`,'i'),'').trim().slice(0,4000);
    return [{id:createHash('sha256').update(url).digest('hex').slice(0,16),title:cleanTitle,description:description||'No excerpt provided by this RSS feed.',url,publisher,publisherUrl:plain(item.source?.['@_url']) || (source.id!=='google'?url:''),source:source.id,publishedAt,category:inferCategory(cleanTitle,description)}];
 });
}
...
export function createFeedReader({fetcher=fetch,now=Date.now}={}){
 ...
 async function readSource(source){
  ...
  states.set(source.id,{items,failures:0,nextAttempt:0,lastSuccessAt:new Date(now()).toISOString(),responseMs:Date.now()-started});
  return {source:{...metadata,status:'ok',lastSuccessAt:new Date(now()).toISOString(),responseMs:Date.now()-started,articlesFound:items.length},items};
  }catch(error){
 ...
 async function run(){
  ...
  const seen=new Set();const rawItems=results.flatMap(r=>r.items).sort((a,b)=>Date.parse(b.publishedAt)-Date.parse(a.publishedAt)).filter(item=>{
  const key=item.url || item.title.toLowerCase().replace(/[^a-z0-9]/g,'');if(seen.has(key))return false;seen.add(key);return true;
  }).slice(0,160);
  const items=clusterStories(rawItems);
