...
export const sources = [
 ...
 {id:'reddit',subreddit:'yeat_',name:'Reddit · r/yeat_',url:'https://www.reddit.com/r/yeat_/new/.rss?limit=30'},
];
...
export function normalize(xml, source){
 ...
 if(source.id==='reddit'){
  if(!document.feed) throw new Error('Invalid Atom response');
...
export function createFeedReader({fetcher=fetch,now=Date.now}={}){
 ...
 async function readSource(source){
  ...
  try{
   const response=await fetcher(source.url,{signal:AbortSignal.timeout(8000),headers:{'User-Agent':'LyfeNewsTracker/1.0 (public RSS reader)','Accept':'application/atom+xml, application/rss+xml, application/xml, text/xml'}});
   if(!response.ok){
   ...
   const items=normalize(await response.text(),source);
